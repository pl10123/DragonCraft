...
public class PacketPipeline extends MessageToMessageCodec<FMLProxyPacket, AbstractPacket> {
    ...
    public boolean registerPacket(Class<? extends AbstractPacket> clazz) {
        if (this.packets.size() > 256) {
        	System.err.println("THIS PACKET IS TOO LARGE IN SIZE!");
            return false;
        }

        if (this.packets.contains(clazz)) {
        	System.err.println("THIS PACKET IS ALREADY REGISTERED!!");
            return false;
        }

        if (this.isPostInitialised) {
        	System.err.println("THE PACKET WAS REGISTERED TOO LATE!!");
            return false;
        }
    ...
    public void initialise() {
        this.channels = NetworkRegistry.INSTANCE.newChannel(Core.MODID, this);
        registerPackets();
    }
    
    public void registerPackets(){
    	this.registerPacket(OpenGuiPacket.class);
    	this.registerPacket(SyncPlayerProperties.class);
    }
